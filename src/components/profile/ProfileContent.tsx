/**
 * PrismSplit Profile Content Component
 * 
 * Shared component used by both tab and non-tab profile screens.
 */
  ...
  return (
    <YStack flex={1}>
      ...
      <Card variant="surface" marginBottom="$6">
        <XStack alignItems="center" gap={showBackButton ? "$3" : "$4"} marginBottom="$4">
          <Avatar
            name={user?.full_name || 'User'}
            colorIndex={user?.color_index || 0}
            size={showBackButton ? "lg" : "xl"}
          />
          <YStack flex={1}>
            <Text fontSize={showBackButton ? 18 : 20} fontWeight="600" color={themeColors.textPrimary}>
              {user?.full_name || 'User'}
            </Text>
